...
import { setemail, setpassword, setloading, setLoginData } from '../redux/authslice';
...
function Login() {
  ...
  const handelform = async (event) => {
    ...
    if (result.status === 200) {
      ...
      // 🆕 Store token, user, and freeCount
      localStorage.setItem("user", JSON.stringify(user));

      dispatch(setLoginData({
        token: result.token,
        user,
        freeCount: result.freeCount, // ⚡ added
      }));
  ...
  return (
    <div className="container-fluid vh-100 d-flex align-items-center justify-content-center px-3">
      <div className="row shadow-lg w-100" style={{ maxWidth: '1000px' }}>
        <div
          className="col-12 col-lg-6 d-flex flex-column align-items-center justify-content-center text-white p-3"
          ...
          <div className="w-100" style={{ maxWidth: '420px' }}>
            ...
            <form onSubmit={handelform}>
              ...
              <p className="text-center">
                Don’t have an account? <Link className="text-dark fw-semibold" to={"/signup"}>Sign up</Link>
              </p>
            </form>
          </div>
        </div>
